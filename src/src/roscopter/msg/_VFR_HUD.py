# This Python file uses the following encoding: utf-8
"""autogenerated by genpy from roscopter/VFR_HUD.msg. Do not edit."""
import codecs
import sys
python3 = True if sys.hexversion > 0x03000000 else False
import genpy
import struct


class VFR_HUD(genpy.Message):
  _md5sum = "d675e2b95b45d51cd67453c819986216"
  _type = "roscopter/VFR_HUD"
  _has_header = False  # flag to mark the presence of a Header object
  _full_text = """# Metrics typically displayed on a HUD for fixed wing aircraft        
#airspeed                  : Current airspeed in m/s (float)
#groundspeed               : Current ground speed in m/s (float)
#heading                   : Current heading in degrees, in compass units (0..360, 0=north) (int16_t)  
#throttle                  : Current throttle setting in integer percent, 0 to 100 (uint16_t)  
#alt                       : Current altitude (MSL), in meters (float)
#climb                     : Current climb rate in meters/second (float)             

float32 airspeed 
float32 groundspeed 
int16 heading
uint16 throttle
float32 alt 
float32 climb
"""
  __slots__ = ['airspeed','groundspeed','heading','throttle','alt','climb']
  _slot_types = ['float32','float32','int16','uint16','float32','float32']

  def __init__(self, *args, **kwds):
    """
    Constructor. Any message fields that are implicitly/explicitly
    set to None will be assigned a default value. The recommend
    use is keyword arguments as this is more robust to future message
    changes.  You cannot mix in-order arguments and keyword arguments.

    The available fields are:
       airspeed,groundspeed,heading,throttle,alt,climb

    :param args: complete set of field values, in .msg order
    :param kwds: use keyword arguments corresponding to message field names
    to set specific fields.
    """
    if args or kwds:
      super(VFR_HUD, self).__init__(*args, **kwds)
      # message fields cannot be None, assign default values for those that are
      if self.airspeed is None:
        self.airspeed = 0.
      if self.groundspeed is None:
        self.groundspeed = 0.
      if self.heading is None:
        self.heading = 0
      if self.throttle is None:
        self.throttle = 0
      if self.alt is None:
        self.alt = 0.
      if self.climb is None:
        self.climb = 0.
    else:
      self.airspeed = 0.
      self.groundspeed = 0.
      self.heading = 0
      self.throttle = 0
      self.alt = 0.
      self.climb = 0.

  def _get_types(self):
    """
    internal API method
    """
    return self._slot_types

  def serialize(self, buff):
    """
    serialize message into buffer
    :param buff: buffer, ``StringIO``
    """
    try:
      _x = self
      buff.write(_get_struct_2fhH2f().pack(_x.airspeed, _x.groundspeed, _x.heading, _x.throttle, _x.alt, _x.climb))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))

  def deserialize(self, str):
    """
    unpack serialized message in str into this message instance
    :param str: byte array of serialized message, ``str``
    """
    if python3:
      codecs.lookup_error("rosmsg").msg_type = self._type
    try:
      end = 0
      _x = self
      start = end
      end += 20
      (_x.airspeed, _x.groundspeed, _x.heading, _x.throttle, _x.alt, _x.climb,) = _get_struct_2fhH2f().unpack(str[start:end])
      return self
    except struct.error as e:
      raise genpy.DeserializationError(e)  # most likely buffer underfill


  def serialize_numpy(self, buff, numpy):
    """
    serialize message with numpy array types into buffer
    :param buff: buffer, ``StringIO``
    :param numpy: numpy python module
    """
    try:
      _x = self
      buff.write(_get_struct_2fhH2f().pack(_x.airspeed, _x.groundspeed, _x.heading, _x.throttle, _x.alt, _x.climb))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))

  def deserialize_numpy(self, str, numpy):
    """
    unpack serialized message in str into this message instance using numpy for array types
    :param str: byte array of serialized message, ``str``
    :param numpy: numpy python module
    """
    if python3:
      codecs.lookup_error("rosmsg").msg_type = self._type
    try:
      end = 0
      _x = self
      start = end
      end += 20
      (_x.airspeed, _x.groundspeed, _x.heading, _x.throttle, _x.alt, _x.climb,) = _get_struct_2fhH2f().unpack(str[start:end])
      return self
    except struct.error as e:
      raise genpy.DeserializationError(e)  # most likely buffer underfill

_struct_I = genpy.struct_I
def _get_struct_I():
    global _struct_I
    return _struct_I
_struct_2fhH2f = None
def _get_struct_2fhH2f():
    global _struct_2fhH2f
    if _struct_2fhH2f is None:
        _struct_2fhH2f = struct.Struct("<2fhH2f")
    return _struct_2fhH2f
